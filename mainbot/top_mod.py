    ...
    if (message.text == "Назад"):
        ...
        try:
            ...
            if (number > 10):
                ...
                bot.send_message(message.chat.id, "Ебанутый? Ты бы еще комплексное число ввел блядь!")
                top(bot, message)
            else:
                top_anekov = top_anek_bd_select(number -1)
                if not top_anekov:
                    bot.send_message(message.chat.id, "Нет анекдота с таким номером")
                    top(bot, message)
                else:
            ...
            bot.send_message(message.chat.id, "Сорян, сейчас я отвечу только на номер или \"Назад\"")
            top(bot, message)
